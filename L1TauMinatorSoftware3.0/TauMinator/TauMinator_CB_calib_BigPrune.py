...
if __name__ == "__main__" :
    ...
    if options.train:
        ...
        middleMan = keras.Input(shape=122, name='middleMan')

        x = layers.Dense(512, use_bias=False, name="DNNlayer1")(middleMan)
        x = layers.Activation('relu', name='RELU_DNNlayer1')(x)
        # x = layers.Dense(128, use_bias=False, name="DNNlayer2")(x)
        ...
        # x = layers.Dense(128, use_bias=False, name="DNNlayer4")(x)
        ...
        callbacks = [tf.keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=0.0000001, mode='min', patience=10, verbose=1, restore_best_weights=True),
                     tf.keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=3, verbose=1),
                     pruning_callbacks.UpdatePruningStep()]

        if options.dm_weighted:
            history = TauMinatorModelPruned.fit(CNNprediction, Ycal, epochs=500, batch_size=1024, verbose=1, validation_split=0.10, callbacks=callbacks, sample_weight=dm_weights)

        elif options.pt_weighted:
            history = TauMinatorModelPruned.fit(CNNprediction, Ycal, epochs=500, batch_size=1024, verbose=1, validation_split=0.10, callbacks=callbacks, sample_weight=pt_weights)

        else:
            history = TauMinatorModelPruned.fit(CNNprediction, Ycal, epochs=500, batch_size=1024, verbose=1, validation_split=0.10, callbacks=callbacks)
    ...
    plt.grid(linestyle='dotted')
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauMinator_CB_calib_plots/predictions_comparison.pdf')
    plt.close()
