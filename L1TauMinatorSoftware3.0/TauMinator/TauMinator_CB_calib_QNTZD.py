...
def inspectWeights(model, which):
    ...
    plt.savefig(outdir+'/TauMinator_CB_calib'+tag+'_plots/modelSparsity'+which+'.pdf')
    plt.close()
...
if __name__ == "__main__" :
    ...
    parser.add_option('--sparsityCNN',  dest='sparsityCNN',                      default=0.5, type=float)
    ...
    os.system('mkdir -p '+outdir+'/TauMinator_CB_calib'+tag+'_plots')
    ...
    if options.pt_weighted:
        ...
        plt.xlim(0,175)
        mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
        plt.savefig(outdir+'/TauMinator_CB_calib'+tag+'_plots/pt_gentau.pdf')
        plt.close()
    ...
    if options.train:
        ...
        sys.stdout = Logger(outdir+'/TauMinator_CB_calib'+tag+'_plots/training_calib.log')
        print(options)

        middleMan = keras.Input(shape=24+2, name='middleMan')

        x = QDense(64, use_bias=False, kernel_quantizer='quantized_bits(6,0,alpha=1)', name='CALlayer1')(middleMan)
        x = QActivation('quantized_relu(9,6)', name='RELU_CALlayer1')(x)
        x = QDense(64, use_bias=False, kernel_quantizer='quantized_bits(6,0,alpha=1)', name='CALlayer2')(x)
        x = QActivation('quantized_relu(9,6)', name='RELU_CALlayer2')(x)
        x = QDense(1, use_bias=False, kernel_quantizer='quantized_bits(6,0,alpha=1)', name="CALout")(x)
        ...
        CNN = keras.models.load_model(outdir+'/CNNmodel_QNTZD_sparsity'+str(options.sparsityCNN), compile=False)
        ...
        # cmsml.tensorflow.save_graph(outdir+'/CMSSWmodels'+tag+'/DNNcalib_CB.pb', TauMinatorModelPruned, variables_to_constants=True)
        # cmsml.tensorflow.save_graph(outdir+'/CMSSWmodels'+tag+'/DNNcalib_CB.pb.txt', TauMinatorModelPruned, variables_to_constants=True)
        ...
        for metric in history.history.keys():
            if metric == 'lr':
                ...
                plt.yscale('log')
                mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
                plt.savefig(outdir+'/TauMinator_CB_calib'+tag+'_plots/'+metric+'.pdf')
                plt.close()
    ...
    train_calib = TauMinatorModelPruned.predict(CNN([X1, X2])) #* 256.
    valid_calib = TauMinatorModelPruned.predict(CNN([X1_valid, X2_valid])) #* 256.
    ...
    plt.grid(linestyle='dotted')
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauMinator_CB_calib'+tag+'_plots/predictions_comparison.pdf')
    plt.close()
    ...
    plt.grid(linestyle='dotted')
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauMinator_CB_calib'+tag+'_plots/responses_comparison.pdf')
    plt.close()
    ...
    plt.xlim(-0.1,5)
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauMinator_CB_calib'+tag+'_plots/response_vs_eta_comparison.pdf')
    plt.close()
    ...
    plt.xlim(-0.1,5)
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauMinator_CB_calib'+tag+'_plots/response_vs_phi_comparison.pdf')
    plt.close()
    ...
    plt.xlim(0.0,2.0)
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauMinator_CB_calib'+tag+'_plots/response_vs_pt_comparison.pdf')
    plt.close()
    ...
    plt.grid()
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauMinator_CB_calib'+tag+'_plots/GenToCalibL1_pt.pdf')
    plt.close()
    ...
    plt.grid()
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauMinator_CB_calib'+tag+'_plots/GenToUncalinL1_pt.pdf')
    plt.close()
    ...
    for ibin in np.sort(dfValid['gen_pt_bin'].unique()):
        ...
        plt.grid(linestyle='dotted')
        mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
        plt.savefig(outdir+'/TauMinator_CB_calib'+tag+'_plots/responses_comparison_'+str(ledge)+'pt'+str(uedge)+'.pdf')
        plt.close()
    ...
    plt.ylim(0.5,1.5)
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauMinator_CB_calib'+tag+'_plots/scale_vs_l1pt.pdf')
    plt.close()
    ...
    plt.ylim(0.5,1.5)
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauMinator_CB_calib'+tag+'_plots/scale_vs_pt.pdf')
    plt.close()
    ...
    plt.ylim(0.1,0.4)
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauMinator_CB_calib'+tag+'_plots/resolution_vs_pt.pdf')
    plt.close()


    ############################## Compare to non-pruned non-quantized model ##############################

    fullCNN = keras.models.load_model(outdir.replace('QNTZD','')+'/CNNmodel_sparsity0.0', compile=False)
    fullCAL = keras.models.load_model(outdir.replace('QNTZD','')+'/CAL_DNNmodel_sparsity0.0', compile=False)

    full_valid_calib = fullCAL.predict(fullCNN([X1_valid, X2_valid])) #* 256.
    
    plt.figure(figsize=(10,10))
    plt.hist(full_valid_calib,  label=r'Full model prediction',    color='red', lw=2, density=True, histtype='step', alpha=0.7, bins=np.arange(18,151,3))
    plt.hist(valid_calib,       label=r'Quantized model prediction', color='green', lw=2, density=True, histtype='step', alpha=0.7, bins=np.arange(18,151,3))
    plt.xlabel(r'$p_{T}^{L1 \tau}$')
    plt.ylabel(r'a.u.')
    plt.yscale('log')
    plt.legend(loc = 'upper right', fontsize=16)
    plt.grid(linestyle='dotted')
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauMinator_CB_calib'+tag+'_plots/predictions_comparison_vsFull.pdf')
    plt.close()


    fulldfValid = pd.DataFrame()
    fulldfValid['uncalib_pt'] = np.sum(X1_valid, (1,2,3)).ravel() #* 256.
    fulldfValid['calib_pt']   = full_valid_calib.ravel()
    fulldfValid['gen_pt']     = Y_valid[:,0].ravel()
    fulldfValid['gen_eta']    = Y_valid[:,2].ravel()
    fulldfValid['gen_phi']    = Y_valid[:,3].ravel()

    # PLOTS INCLUSIVE
    plt.figure(figsize=(10,10))
    plt.hist(fulldfValid['calib_pt']/fulldfValid['gen_pt'],   bins=np.arange(0.05,5,0.1), label=r'Full model response : $\mu$ = %.2f, $\sigma$ =  %.2f' % (np.mean(fulldfValid['calib_pt']/fulldfValid['gen_pt']), np.std(fulldfValid['calib_pt']/fulldfValid['gen_pt'])), color='red', lw=2, density=True, histtype='step', alpha=0.7)
    plt.hist(dfValid['calib_pt']/dfValid['gen_pt'],   bins=np.arange(0.05,5,0.1), label=r'Quantized model response : $\mu$ = %.2f, $\sigma$ =  %.2f' % (np.mean(dfValid['calib_pt']/dfValid['gen_pt']), np.std(dfValid['calib_pt']/dfValid['gen_pt'])), color='green',lw=2, density=True, histtype='step', alpha=0.7)
    plt.xlabel(r'$p_{T}^{L1 \tau} / p_{T}^{Gen \tau}$')
    plt.ylabel(r'a.u.')
    plt.legend(loc = 'upper right', fontsize=16)
    plt.grid(linestyle='dotted')
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauMinator_CB_calib'+tag+'_plots/responses_comparison_vsFull.pdf')
    plt.close()
