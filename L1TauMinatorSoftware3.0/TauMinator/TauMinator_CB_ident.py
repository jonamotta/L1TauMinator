...
if __name__ == "__main__" :
    ...
    if options.train:
        ...
        TauMinatorModelPruned = keras.Model(inputs=[images, positions], outputs=TauIdentified, name='TauMinator_CB_indent')
        ...
        cmsml.tensorflow.save_graph(outdir+'/CMSSWmodels'+tag+'/CNNmodel_CB.pb', CNNmodel, variables_to_constants=True)
        cmsml.tensorflow.save_graph(outdir+'/CMSSWmodels'+tag+'/CNNmodel_CB.pb.txt', CNNmodel, variables_to_constants=True)
        ...
        cmsml.tensorflow.save_graph(outdir+'/CMSSWmodels'+tag+'/DNNident_CB.pb', ID_DNNmodel, variables_to_constants=True)
        cmsml.tensorflow.save_graph(outdir+'/CMSSWmodels'+tag+'/DNNident_CB.pb.txt', ID_DNNmodel, variables_to_constants=True)
    ...
    # test_sel = (((Y_valid[:,0] > 20) & (Y_valid[:,1] == 1)) | (Y_valid[:,1] == 0))
    # X1_valid = X1_valid[test_sel]
    # X2_valid = X2_valid[test_sel]
    # Y_valid = Y_valid[test_sel]
    # Yid_valid = Yid_valid[test_sel]
    ...
    plt.hist(df[df['true']==0]['score'], bins=np.arange(0,1,0.02), label=r'Background', color='red', density=True, histtype='step', lw=3, zorder=1)
    plt.hist(df[df['true']==1]['score'], bins=np.arange(0,1,0.02), label=r'$\tau_h$ signal', color='green', density=True, histtype='step', lw=3, zorder=2)
    plt.grid(linestyle=':')
    ...
    plt.savefig(outdir+'/TauMinator_CB_ident'+tag+'_plots/DP_cnn_score.pdf')
    plt.close()
    ...
    plt.figure(figsize=(10,10))
    plt.scatter(df[df['true']==1]['score'], df[df['true']==1]['gen_pt'], color='green', alpha=0.2)
    plt.grid(linestyle=':')
    # plt.xlim(0.5,1.001)
    plt.ylim(17,60)
    # plt.yscale('log')
    ...
    plt.ylabel(r'$p_{T}^{Gen. \tau}$')
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauMinator_CB_ident'+tag+'_plots/CNN_score_vs_genpt.pdf')
    plt.close()
