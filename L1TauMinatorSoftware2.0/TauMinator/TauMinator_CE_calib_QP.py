...
def inspectWeights(model, which):
    ...
    plt.savefig(outdir+'/TauMinator_CE_calib_plots/modelSparsity'+which+'.pdf')
    plt.close()
...
if __name__ == "__main__" :
    ...
    outdir += tag
    os.system('mkdir -p '+outdir+'/TauMinator_CE_calib_plots')
    ...
    if options.pt_weighted:
        ...
        plt.xlim(0,175)
        mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
        plt.savefig(outdir+'/TauMinator_CE_calib_plots/pt_gentau.pdf')
        plt.close()
    ...
    if options.train:
        ...
        sys.stdout = Logger(outdir+'/TauMinator_CE_calib_plots/training_calib.log')
        print(options)

        middleMan = keras.Input(shape=26+len(features2use), name='middleMan')

        x = QDense(16, use_bias=False, kernel_quantizer='quantized_bits(6,0,alpha=1)', name='CALlayer1')(middleMan)
        x = QActivation('quantized_relu(9,6)', name='RELU_CALlayer1')(x)
        x = QDense(8, use_bias=False, kernel_quantizer='quantized_bits(6,0,alpha=1)', name='CALlayer2')(x)
        ...
        TauMinatorModel = keras.Model(inputs=middleMan, outputs=TauCalibrated, name='TauMinator_CE_calib')
        ...
        if options.dm_weighted:
            history = TauMinatorModelPruned.fit(CNNprediction, Ycal, epochs=500, batch_size=batch_size, verbose=1, validation_split=0.25, callbacks=callbacks, sample_weight=dm_weights)

        elif options.pt_weighted:
            history = TauMinatorModelPruned.fit(CNNprediction, Ycal, epochs=500, batch_size=batch_size, verbose=1, validation_split=0.25, callbacks=callbacks, sample_weight=pt_weights)

        else:
            history = TauMinatorModelPruned.fit(CNNprediction, Ycal, epochs=500, batch_size=batch_size, verbose=1, validation_split=0.25, callbacks=callbacks)

        TauMinatorModelPruned = strip_pruning(TauMinatorModelPruned)

        TauMinatorModelPruned.save(outdir+'/CAL_DNNmodel', include_optimizer=False)
        # cmsml.tensorflow.save_graph(indir+'/CMSSWmodels'+tag+'/DNNcalib_CE.pb', TauMinatorModelPruned, variables_to_constants=True)
        # cmsml.tensorflow.save_graph(indir+'/CMSSWmodels'+tag+'/DNNcalib_CE.pb.txt', TauMinatorModelPruned, variables_to_constants=True)
        ...
        for metric in history.history.keys():
            if metric == 'lr':
                ...
                plt.yscale('log')
                mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
                plt.savefig(outdir+'/TauMinator_CE_calib_plots/'+metric+'.pdf')
                plt.close()
        ...
        CNN = keras.models.load_model(outdir+'/CNNmodel', compile=False)
        TauMinatorModelPruned = keras.models.load_model(outdir+'/CAL_DNNmodel', compile=False)
    ...
    train_calib = TauMinatorModelPruned.predict(CNN([X1, X2, X3]))
    valid_calib = TauMinatorModelPruned.predict(CNN([X1_valid, X2_valid, X3_valid]))
    ...
    plt.xlim(-0.1,5)
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauMinator_CE_calib_plots/response_vs_eta_comparison.pdf')
    plt.close()
    ...
    plt.xlim(-0.1,5)
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauMinator_CE_calib_plots/response_vs_phi_comparison.pdf')
    plt.close()
    ...
    plt.xlim(0.0,2.0)
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauMinator_CE_calib_plots/response_vs_pt_comparison.pdf')
    plt.close()
    ...
    plt.grid()
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauMinator_CE_calib_plots/GenToUncalinL1_pt.pdf')
    plt.close()
    ...
    for ibin in np.sort(dfValid['gen_pt_bin'].unique()):
        ...
        plt.grid(linestyle='dotted')
        mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
        plt.savefig(outdir+'/TauMinator_CE_calib_plots/responses_comparison_'+str(ledge)+'pt'+str(uedge)+'.pdf')
        plt.close()
    ...
    plt.ylim(0.5,1.5)
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauMinator_CE_calib_plots/scale_vs_pt.pdf')
    plt.close()
    ...
    plt.ylim(0.1,0.4)
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauMinator_CE_calib_plots/resolution_vs_pt.pdf')
    plt.close()
    ...
    for ibin in np.sort(dfValid['gen_pt_bin'].unique()):
        ...
        plt.grid(linestyle='dotted')
        mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
        plt.savefig(outdir+'/TauMinator_CE_calib_plots/responses_comparison_'+str(ledge)+'pt'+str(uedge)+'_corr.pdf')
        plt.close()
    ...
    plt.ylim(0.5,1.5)
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauMinator_CE_calib_plots/scale_vs_pt_corr.pdf')
    plt.close()
    ...
    plt.ylim(0.1,0.4)
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauMinator_CE_calib_plots/resolution_vs_pt_corr.pdf')
    plt.close()
    ...
    plt.grid(linestyle='dotted')
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauMinator_CE_calib_plots/responses_comparison_corr.pdf')
    plt.close()
