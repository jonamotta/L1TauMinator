...
if __name__ == "__main__" :
    ...
    outdir = '/data_CMS/cms/'+user+'/Phase2L1T/'+options.date+'_v'+options.v+'/TauMinator_CB_cltw'+options.caloClNxM+'_Training_QP'+options.inTag
    tag = ''
    ...
    if options.train:
        ...
        # cmsml.tensorflow.save_graph(indir+'/CMSSWmodels'+tag+'/DNNcalib_CB.pb', TauMinatorModelPruned, variables_to_constants=True)
        # cmsml.tensorflow.save_graph(indir+'/CMSSWmodels'+tag+'/DNNcalib_CB.pb.txt', TauMinatorModelPruned, variables_to_constants=True)
    ...
    train_calib = TauMinatorModelPruned.predict(CNN([X1, X2]))
    valid_calib = TauMinatorModelPruned.predict(CNN([X1_valid, X2_valid]))
