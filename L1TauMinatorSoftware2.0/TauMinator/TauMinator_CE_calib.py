...
if __name__ == "__main__" :
    ...
    if options.train:
        ...
        if options.dm_weighted:
            history = TauMinatorModel.fit(CNNprediction, Ycal, epochs=500, batch_size=1024, verbose=1, validation_split=0.25, callbacks=callbacks, sample_weight=dm_weights)

        elif options.pt_weighted:
            history = TauMinatorModel.fit(CNNprediction, Ycal, epochs=500, batch_size=1024, verbose=1, validation_split=0.25, callbacks=callbacks, sample_weight=pt_weights)

        else:
            history = TauMinatorModel.fit(CNNprediction, Ycal, epochs=500, batch_size=1024, verbose=1, validation_split=0.25, callbacks=callbacks)
    ...
    train_calib = TauMinatorModel.predict(CNN([X1, X2, X3]))
    valid_calib = TauMinatorModel.predict(CNN([X1_valid, X2_valid, X3_valid]))
