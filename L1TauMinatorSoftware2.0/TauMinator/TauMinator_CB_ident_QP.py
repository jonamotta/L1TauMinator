...
if __name__ == "__main__" :
    ...
    if options.train:
        ...
        if options.dm_weighted:
            history = TauMinatorModelPruned.fit([X1, X2], Yid, epochs=500, batch_size=batch_size, shuffle=True, verbose=1, validation_split=0.25, callbacks=callbacks, sample_weight=dm_weights)
        
        else:
            history = TauMinatorModelPruned.fit([X1, X2], Yid, epochs=500, batch_size=batch_size, shuffle=True, verbose=1, validation_split=0.25, callbacks=callbacks)

        TauMinatorModelPruned = strip_pruning(TauMinatorModelPruned)

        TauMinatorModelPruned.save(outdir + '/TauMinator_CB_ident')
        ...
        TauMinatorModelPruned = keras.models.load_model(outdir+'/TauMinator_CB_ident', compile=False)
    ...
    valid_ident_dm0 = TauMinatorModelPruned.predict([X1_valid_dm0, X2_valid_dm0])
    ...
    valid_ident_dm1 = TauMinatorModelPruned.predict([X1_valid_dm1, X2_valid_dm1])
    ...
    valid_ident_dm10 = TauMinatorModelPruned.predict([X1_valid_dm10, X2_valid_dm10])
    ...
    valid_ident_dm11 = TauMinatorModelPruned.predict([X1_valid_dm11, X2_valid_dm11])
