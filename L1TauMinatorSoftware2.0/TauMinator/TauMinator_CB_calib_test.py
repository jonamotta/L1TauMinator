...
if __name__ == "__main__" :
    ...
    PT = np.sum(np.sum(np.sum(X1, axis=3), axis=2), axis=1).ravel()
    NTT = np.sum(np.sum(np.sum(X1>0, axis=3), axis=2), axis=1).ravel()
    ETA = X2[:,0]
    PHI = X2[:,1]
    ...
    PT = (PT - PTmean)/PTstd
    NTT = (NTT - NTTmean)/NTTstd
    ETA = (ETA - ETAmean)/ETAstd
    PT = (PHI - PHImean)/PHIstd

    IN = np.stack([PT, NTT, ETA, PHI], axis=-1)

    # IN = np.array(IN[[0,2]])
    # Ycal = Ycal[[0,2]].reshape(-1,1)

    # IN = np.array([IN[0]])
    # Ycal = Ycal[0].reshape(-1,1)
    ...
    if options.train:
        ...
        middleMan = keras.Input(shape=4, name='middleMan')

        x = layers.Dense(32, use_bias=True, name="DNNlayer1")(middleMan)
        x = layers.Activation('relu', name='RELU_DNNlayer1')(x)
        x = layers.Dense(64, use_bias=True, name="DNNlayer2")(x)
        ...
        #     print('feature', i)
        #     plt.figure(figsize=(10,10))
        #     plt.hist(np.array(CNNprediction[:,i]))
        #     plt.savefig('norm/feat'+str(i)+'.pdf')
        #     plt.close()
        ...
        if options.dm_weighted:
            history = TauMinatorModel.fit(IN, Ycal, epochs=1, batch_size=1024, verbose=1, validation_split=0.25, callbacks=callbacks, sample_weight=dm_weights)

        elif options.pt_weighted:
            history = TauMinatorModel.fit(IN, Ycal, epochs=1, batch_size=1024, verbose=1, validation_split=0.25, callbacks=callbacks, sample_weight=pt_weights)

        else:
            history = TauMinatorModel.fit(IN, Ycal, epochs=50, batch_size=1024, verbose=1, validation_split=0.25, callbacks=callbacks)
    ...
    PT = np.sum(np.sum(np.sum(X1_valid, axis=3), axis=2), axis=1).ravel()
    NTT = np.sum(np.sum(np.sum(X1_valid>0, axis=3), axis=2), axis=1).ravel()
    ETA = X2_valid[:,0]
    PHI = X2_valid[:,1]
    ...
    IN_valid = np.stack([PT, NTT, ETA, PHI], axis=-1)

    train_calib = TauMinatorModel.predict(IN)
    valid_calib = TauMinatorModel.predict(IN_valid)
