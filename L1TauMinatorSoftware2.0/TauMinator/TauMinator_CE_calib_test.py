...
if __name__ == "__main__" :
    ...
    if options.train:
        ...
        middleMan = keras.Input(shape=len(features2use), name='middleMan')

        x = layers.Dense(64, use_bias=True, name="DNNlayer1")(middleMan)
        x = layers.Activation('relu', name='RELU_DNNlayer1')(x)
        x = layers.Dense(128, use_bias=True, name="DNNlayer2")(x)
        x = layers.Activation('relu', name='RELU_DNNlayer2')(x)
        x = layers.Dense(32, use_bias=True, name="DNNlayer3")(x)
        x = layers.Activation('relu', name='RELU_DNNlayer3')(x)
        x = layers.Dense(1, use_bias=True, name="DNNout", activation='linear')(x)
        ...
        if options.dm_weighted:
            history = TauMinatorModel.fit(X3, Ycal, epochs=500, batch_size=1024, verbose=1, validation_split=0.25, callbacks=callbacks, sample_weight=dm_weights)

        elif options.pt_weighted:
            history = TauMinatorModel.fit(X3, Ycal, epochs=500, batch_size=1024, verbose=1, validation_split=0.25, callbacks=callbacks, sample_weight=pt_weights)

        else:
            history = TauMinatorModel.fit(X3, Ycal, epochs=500, batch_size=1024, verbose=1, validation_split=0.25, callbacks=callbacks)

        TauMinatorModel.save(outdir+'/CAL_DNNmodel', include_optimizer=False)
        cmsml.tensorflow.save_graph(indir+'/CMSSWmodels'+tag+'/DNNcalib_CE.pb', TauMinatorModel, variables_to_constants=True)
        cmsml.tensorflow.save_graph(indir+'/CMSSWmodels'+tag+'/DNNcalib_CE.pb.txt', TauMinatorModel, variables_to_constants=True)
    ...
    train_calib = TauMinatorModel.predict(X3)
    valid_calib = TauMinatorModel.predict(X3_valid)
