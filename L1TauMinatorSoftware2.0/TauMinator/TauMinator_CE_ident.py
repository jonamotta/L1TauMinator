...
if __name__ == "__main__" :
    ...
    if options.train:
        ...
        if options.dm_weighted:
            history = TauMinatorModel.fit([X1, X2, X3], Yid, epochs=200, batch_size=1024, shuffle=True, verbose=1, validation_split=0.25, callbacks=callbacks, sample_weight=dm_weights)
        
        else:
            history = TauMinatorModel.fit([X1, X2, X3], Yid, epochs=200, batch_size=1024, shuffle=True, verbose=1, validation_split=0.25, callbacks=callbacks)

        TauMinatorModel.save(outdir + '/TauMinator_CE_ident')
        ...
        cl3ds_in = TauMinatorModel.get_layer(name='AssociatedCl3dFeatures').get_output_at(0)
        flat_out = TauMinatorModel.get_layer(name='middleMan').get_output_at(0)
        CNNmodel = tf.keras.Model([image_in, posit_in, cl3ds_in], flat_out, name="TauMinator_CE_conv")
        ...
        y_full = np.array( TauMinatorModel.predict([X1_valid, X2_valid, X3_valid]) )
        ...
        TauMinatorModel = keras.models.load_model(outdir+'/TauMinator_CE_ident', compile=False)
    ...
    train_ident = TauMinatorModel.predict([X1, X2, X3])
    valid_ident = TauMinatorModel.predict([X1_valid, X2_valid, X3_valid])
