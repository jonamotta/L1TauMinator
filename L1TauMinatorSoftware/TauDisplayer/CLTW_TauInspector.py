...
if __name__ == "__main__" :
    ...
    plt.hist(dfValid['uncalib_pt']/dfValid['gen_pt'], label=r'Valid. Uncalibrated response, $\mu$: %.2f, $\sigma$ : %.2f' % (np.mean(dfValid['uncalib_pt']/dfValid['gen_pt']), np.std(dfValid['uncalib_pt']/dfValid['gen_pt'])), color='red',  lw=2, density=True, histtype='step')
    plt.hist(dfTrain['uncalib_pt']/dfTrain['gen_pt'], label=r'Train. Uncalibrated response, $\mu$: %.2f, $\sigma$ : %.2f' % (np.mean(dfTrain['uncalib_pt']/dfTrain['gen_pt']), np.std(dfTrain['uncalib_pt']/dfTrain['gen_pt'])), color='blue', lw=2, density=True, histtype='step')
    ...
    plt.savefig(outdir+'/TauCNNInspector_plots/input_responses_comparison_before_cut.pdf')
    plt.close()

    dfValid = dfValid[(dfValid['uncalib_pt']/dfValid['gen_pt']) < 6].copy(deep=True)
    dfTrain = dfTrain[(dfTrain['uncalib_pt']/dfTrain['gen_pt']) < 6].copy(deep=True)
    print('\nAFTER uncalib_pt/gen_pt\n------------')
    print('dfValid shape ', dfValid.shape)
    print('dfTrain shape ', dfTrain.shape)
    ...
    plt.yscale('log')
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauCNNInspector_plots/input_responses_comparison_after_cut.pdf')
    plt.close()
    ...
    plt.yscale('log')
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauCNNInspector_plots/input_difference_comparison_before_cut.pdf')
    plt.close()

    plt.figure(figsize=(10,10))
    plt.scatter(dfValid['gen_pt'], (dfValid['uncalib_pt']-dfValid['gen_pt']), label=r'Valid', color='red',  marker='o', alpha=0.2)
    plt.scatter(dfTrain['gen_pt'], (dfTrain['uncalib_pt']-dfTrain['gen_pt']), label=r'Train', color='blue', marker='o', alpha=0.2)
    plt.xlabel(r'$p_{T}^{L1 \tau} / p_{T}^{Gen \tau}$')
    ...
    # plt.yscale('log')
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauCNNInspector_plots/input_difference_vs_pt_comparison_before_cut.pdf')
    plt.close()

    dfValid = dfValid[abs(dfValid['uncalib_pt']-dfValid['gen_pt']) < 500].copy(deep=True)
    dfTrain = dfTrain[abs(dfTrain['uncalib_pt']-dfTrain['gen_pt']) < 500].copy(deep=True)
    print('\nAFTER uncalib_pt-gen_pt\n------------')
    print('dfValid shape ', dfValid.shape)
    print('dfTrain shape ', dfTrain.shape)
    ...
    plt.yscale('log')
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauCNNInspector_plots/input_difference_comparison_after_cut.pdf')
    plt.close()
    ...
    # plt.yscale('log')
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauCNNInspector_plots/input_difference_vs_pt_comparison_after_cut.pdf')
    plt.close()


    # dfValid = dfValid[not((dfValid['uncalib_pt'] > 200) & (dfValid['gen_pt'] > 200))].copy(deep=True)
    # dfTrain = dfTrain[not((dfTrain['uncalib_pt'] > 200) & (dfTrain['gen_pt'] > 200))].copy(deep=True)
    # print('\nAFTER uncalib_pt\n------------')
    # print('dfValid shape ', dfValid.shape)
    # print('dfTrain shape ', dfTrain.shape)
