...
if __name__ == "__main__" :
    ...
    outdir = '/data_CMS/cms/motta/Phase2L1T/'+options.date+'_v'+options.v+'/endcap/TauCNNCalibrator'+options.caloClNxM+'Training'+options.inTag
    ...
    X1 = np.load(outdir+'/CLTWimages_endcap_CAL.npz')['arr_0']
    X2 = np.load(outdir+'/CLTWpositions_endcap_CAL.npz')['arr_0']
    X3 = np.load(outdir+'/CL3Dfeatures_endcap_CAL.npz')['arr_0']
    Y = np.load(outdir+'/Y_endcap_CAL.npz')['arr_0']
    ...
    plt.grid()
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauCNNCalibrator_plots/GenToCalibL1_pt.pdf')
    plt.close()
