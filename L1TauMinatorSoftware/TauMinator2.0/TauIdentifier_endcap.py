...
if __name__ == "__main__" :
    ...
    indir = '/data_CMS/cms/motta/Phase2L1T/'+options.date+'_v'+options.v+'/endcap'
    outdir = '/data_CMS/cms/motta/Phase2L1T/'+options.date+'_v'+options.v+'/endcap/TauCNNIdentifier'+options.caloClNxM+'Training'+options.inTag
    ...
    #
    # X3 is (None, 18)
    #       for the 18 available features for CL3Ds
    #       cl3d_pt, cl3d_energy, cl3d_localAbsEta, cl3d_phi, cl3d_showerlength, cl3d_coreshowerlength, cl3d_firstlayer, cl3d_seetot, cl3d_seemax, cl3d_spptot, cl3d_sppmax, cl3d_szz, cl3d_srrtot, cl3d_srrmax, cl3d_srrmean, cl3d_hoe, cl3d_localAbsMeanZ, cl3d_quality
    #
    # Y is (None, 5)
    #       target: visPt, visEta, visPhi, DM, PID

    X1 = np.load(indir+'/CLTWimages_endcap_ID.npz')['arr_0']
    X2 = np.load(indir+'/CLTWpositions_endcap_ID.npz')['arr_0']
    X3 = np.load(indir+'/CL3Dfeatures_endcap_ID.npz')['arr_0']
    Y = np.load(indir+'/Y_endcap_ID.npz')['arr_0']

    X3 = X3[:,[0,2,4,5,6,7,11,12,14,15,16]] # need to include also cl3d_pt for the scaler to work
    scaler = load_obj('/data_CMS/cms/motta/Phase2L1T/2022_10_04_v12/TauDNNOptimization/dnn_features_scaler.pkl')
    X3 = scaler.transform(X3)
    X3 = X3[:,1:11] # remove cl3d_pt from the features for identfication
    ...
    if options.train:
        ...
        cl3dFeats = keras.Input(shape = 10, name='AssociatedCl3dFeatures')
        ...
        TauIdentifierModel = keras.Model([images, positions, cl3dFeats], TauIdentified, name='TauCNNIdentifier')
        ...
        history = TauIdentifierModel.fit([X1, X2, X3], Y, epochs=20, batch_size=1024, verbose=1, validation_split=0.25, callbacks=callbacks)
    ...
    train_ident = TauIdentifierModel.predict([X1, X2, X3])
    FPRtrain, TPRtrain, THRtrain = metrics.roc_curve(Y, train_ident)
    AUCtrain = metrics.roc_auc_score(Y, train_ident)

    valid_ident = TauIdentifierModel.predict([X1_valid, X2_valid, X3_valid])
