...
if __name__ == "__main__" :
    ...
    outdir = indir+'/TauCNNEvaluator'+options.caloClNxM+options.inTag

    TauCalibratorModel = keras.models.load_model(indir+'/TauCNNCalibrator'+options.caloClNxM+'Training'+options.modelsTag+'/TauCNNCalibrator', compile=False)
    TauIdentifierModel = keras.models.load_model(indir+'/TauCNNIdentifier'+options.caloClNxM+'Training'+options.modelsTag+'/TauCNNIdentifier', compile=False)

    X1 = np.load(outdir+'/X_Rate_CNN_'+options.caloClNxM+'_forEvaluator.npz')['arr_0']
    X2 = np.load(outdir+'/X_Rate_Dense_'+options.caloClNxM+'_forEvaluator.npz')['arr_0']
    Y = np.load(outdir+'/Y_Rate_'+options.caloClNxM+'_forEvaluator.npz')['arr_0']

    Xidentified = TauIdentifierModel.predict([X1, X2])
    Xcalibrated = TauCalibratorModel.predict([X1, X2])

    wp_dict = load_obj(indir+'/TauCNNEvaluator'+options.caloClNxM+options.modelsTag+'/TauMinatorCNN_WPs.pkl')
    WP99 = wp_dict['wp99']
    WP95 = wp_dict['wp95']
    WP90 = wp_dict['wp90']
    ...
    mapping_dict = load_obj(indir+'/TauCNNEvaluator'+options.caloClNxM+options.modelsTag+'/online2offline_mapping.pkl')
    ...
    for WP in ['99', '95', '90']:
        ...
        plt.ylabel('Rate [kHz]')
        plt.title('Single Tau Rate - WP'+WP, fontsize=15)
        ...
        plt.xlabel('Offline threshold [GeV]')
        plt.ylabel('Rate [kHz]')
        plt.title('Double Tau Rate - WP'+WP, fontsize=15)
        plt.subplots_adjust(bottom=0.12)
