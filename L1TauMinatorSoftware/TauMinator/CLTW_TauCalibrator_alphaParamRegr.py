...
def inspectWeights(model, which):
    ...
    plt.savefig(outdir+'/TauCNNCalibrator_alpha_plots/modelSparsity'+which+'.pdf')
    plt.close()
...
if __name__ == "__main__" :
    ...
    os.system('mkdir -p '+outdir+'/TauCNNCalibrator_alpha_plots')
    ...
    target = ( Y[:,0].ravel() / np.sum(np.sum(np.sum(X1, axis=3), axis=2), axis=1).ravel() ).reshape(-1,1)
    ...
    if options.train:
        ...
        sys.stdout = Logger(outdir+'/TauCNNCalibrator_alpha_plots/training.log')
        print(options)
        ...
        callbacks = [tf.keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=0.01, mode='min', patience=10, verbose=1, restore_best_weights=True),
                     tf.keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=3, verbose=1)]

        history = TauCalibratorModel.fit(CNNprediction, target, epochs=200, batch_size=1024, verbose=1, validation_split=0.25, callbacks=callbacks)

        TauCalibratorModel.save(outdir+'/TauCNNCalibrator_alpha')
        ...
        for metric in history.history.keys():
            if metric == 'lr':
                ...
                plt.yscale('log')
                mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
                plt.savefig(outdir+'/TauCNNCalibrator_alpha_plots/'+metric+'.pdf')
                plt.close()
        ...
        CNN = keras.models.load_model(indir+'/TauCNNIdentifier'+options.caloClNxM+'Training'+options.inTagCNN+'/CNNmodel', compile=False)
        TauCalibratorModel = keras.models.load_model(outdir+'/TauCNNCalibrator_alpha', compile=False)
    ...
    dfTrain['alpha']      = train_calib.ravel()
    dfTrain['calib_pt']   = dfTrain['uncalib_pt'] * dfTrain['alpha']
    ...
    dfValid['alpha']      = valid_calib.ravel()
    dfValid['calib_pt']   = dfValid['uncalib_pt'] * dfTrain['alpha']
    ...
    plt.hist(target,             bins=np.arange(0,5,0.1), label=r'Target $\alpha$ param. : $\mu$ = %.2f, $\sigma$ =  %.2f' % (np.mean(target), np.std(target)),                     color='red',  lw=2, density=True, histtype='step', alpha=0.7)
    plt.hist(dfTrain['alpha'],   bins=np.arange(0,5,0.1), label=r'Train. $\alpha$ param. : $\mu$ = %.2f, $\sigma$ =  %.2f' % (np.mean(dfTrain['alpha']), np.std(dfTrain['alpha'])), color='blue', lw=2, density=True, histtype='step', alpha=0.7)
    plt.hist(dfValid['alpha'],   bins=np.arange(0,5,0.1), label=r'Valid. $\alpha$ param. : $\mu$ = %.2f, $\sigma$ =  %.2f' % (np.mean(dfValid['alpha']), np.std(dfValid['alpha'])), color='green',lw=2, density=True, histtype='step', alpha=0.7)
    plt.xlabel(r'$p_{T}^{L1 \tau} / p_{T}^{Gen \tau}$')
    plt.ylabel(r'a.u.')
    plt.legend(loc = 'upper right', fontsize=16)
    plt.grid(linestyle='dotted')
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauCNNCalibrator_alpha_plots/alpha_param.pdf')
    plt.close()
    ...
    plt.grid(linestyle='dotted')
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauCNNCalibrator_alpha_plots/responses_comparison.pdf')
    plt.close()
    ...
    plt.grid(linestyle='dotted')
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauCNNCalibrator_alpha_plots/uncalibrated_DM_responses_comparison.pdf')
    plt.close()
    ...
    plt.grid(linestyle='dotted')
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauCNNCalibrator_alpha_plots/calibrated_DM_responses_comparison.pdf')
    plt.close()
    ...
    plt.xlim(-0.1,5)
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauCNNCalibrator_alpha_plots/response_vs_eta_comparison.pdf')
    plt.close()
    ...
    plt.xlim(-0.1,5)
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauCNNCalibrator_alpha_plots/response_vs_phi_comparison.pdf')
    plt.close()
    ...
    plt.xlim(0.0,2.0)
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauCNNCalibrator_alpha_plots/response_vs_pt_comparison.pdf')
    plt.close()
    ...
    plt.grid()
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauCNNCalibrator_alpha_plots/GenToCalibL1_pt.pdf')
    plt.close()
    ...
    plt.grid()
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauCNNCalibrator_alpha_plots/GenToUncalinL1_pt.pdf')
    plt.close()
...
#    plt.savefig(outdir+'/TauCNNCalibrator_alpha_plots/shap0.pdf')
#    plt.close()
#
#    plt.figure(figsize=(10,10))
...
#    plt.savefig(outdir+'/TauCNNCalibrator_alpha_plots/shap1.pdf')
#    plt.close()
