...
if __name__ == "__main__" :
    ...
    os.system('mkdir -p '+outdir+'/TauCNNQCalibrator'+sparsityTag+'Pruning_plots')
    ...
    if options.train:
        ...
        sys.stdout = Logger(outdir+'/TauCNNQCalibrator'+sparsityTag+'Pruning_plots/training.log')
        print(options)
        ...
        TauQCalibratorModel_base = keras.Model([images, positions], TauCalibrated, name='TauCNNCalibrator')
        ...
        # print_qmodel_summary(TauQCalibratorModelPruned)        
        # exit()

        ############################## Model training ##############################

        history = TauQCalibratorModelPruned.fit([X1, X2], Y[:,0].reshape(-1,1), epochs=20, batch_size=1024, verbose=1, validation_split=0.1, callbacks=callbacks)
        ...
        TauQCalibratorModelPruned = keras.models.load_model('/data_CMS/cms/motta/Phase2L1T/'+options.date+'_v'+options.v+'/TauCNNCalibrator'+options.caloClNxM+'Training'+options.inTag+'/TauCNNQCalibrator'+sparsityTag+'Pruned', compile=False)


    ############################## Model validation ##############################

    # load non-pruned model
    TauQCalibratorModel = keras.models.load_model(indir+'/TauQCNNCalibrator', compile=False)
    ...
    train_Qcalib_pruned = TauQCalibratorModelPruned.predict([X1, X2])
    valid_Qcalib_pruned = TauQCalibratorModelPruned.predict([X1_valid, X2_valid])
    
    dfTrain = pd.DataFrame()
    dfTrain['uncalib_pt'] = np.sum(np.sum(np.sum(X1, axis=3), axis=2), axis=1).ravel()
    dfTrain['Qcalib_pt']   = train_Qcalib.ravel()
    dfTrain['Qcalib_pt_pruned']  = train_Qcalib_pruned.ravel()
    ...
    dfValid['Qcalib_pt_pruned']  = valid_Qcalib_pruned.ravel()
    ...
    inspectWeights(TauQCalibratorModelPruned, options.sparsity, 'kernel')
    inspectWeights(TauQCalibratorModelPruned, options.sparsity, 'bias')
    ...
    plt.hist(dfTrain['Qcalib_pt_pruned']/dfTrain['gen_pt'],   bins=np.arange(0,5,0.1), label=r'Train. Quantized Pruned Calibrated response : $\mu$ = %.2f, $\sigma$ =  %.2f' % (np.mean(dfTrain['Qcalib_pt_pruned']/dfTrain['gen_pt']), np.std(dfTrain['Qcalib_pt_pruned']/dfTrain['gen_pt'])), color='blue', lw=2, density=True, histtype='step', alpha=0.7)
    plt.hist(dfValid['Qcalib_pt_pruned']/dfValid['gen_pt'],   bins=np.arange(0,5,0.1), label=r'Valid. Quantized Pruned Calibrated response : $\mu$ = %.2f, $\sigma$ =  %.2f' % (np.mean(dfValid['Qcalib_pt_pruned']/dfValid['gen_pt']), np.std(dfValid['Qcalib_pt_pruned']/dfValid['gen_pt'])), color='green',lw=2, density=True, histtype='step', alpha=0.7)
    plt.xlabel(r'$p_{T}^{L1 \tau} / p_{T}^{Gen \tau}$')
    plt.ylabel(r'a.u.')
    plt.legend(loc = 'upper right', fontsize=16)
    plt.grid(linestyle='dotted')
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauCNNQCalibrator'+sparsityTag+'Pruning_plots/responses_comparison.pdf')
    plt.close()
    ...
    plt.hist(dfTrain['Qcalib_pt_pruned']/dfTrain['gen_pt'],   bins=np.arange(0,5,0.1), label=r'Train. Quantized Pruned Calibrated response : $\mu$ = %.2f, $\sigma$ =  %.2f' % (np.mean(dfTrain['Qcalib_pt_pruned']/dfTrain['gen_pt']), np.std(dfTrain['Qcalib_pt_pruned']/dfTrain['gen_pt'])), color='blue',  ls='--', lw=2, density=True, histtype='step', alpha=0.7)
    plt.hist(dfValid['Qcalib_pt_pruned']/dfValid['gen_pt'],   bins=np.arange(0,5,0.1), label=r'Valid. Quantized Pruned Calibrated response : $\mu$ = %.2f, $\sigma$ =  %.2f' % (np.mean(dfValid['Qcalib_pt_pruned']/dfValid['gen_pt']), np.std(dfValid['Qcalib_pt_pruned']/dfValid['gen_pt'])), color='green', ls='--', lw=2, density=True, histtype='step', alpha=0.7)
    plt.xlabel(r'$p_{T}^{L1 \tau} / p_{T}^{Gen \tau}$')
    plt.ylabel(r'a.u.')
    plt.xlim(0., 2.)
    plt.ylim(0., 3.5)
    ...
    plt.grid(linestyle='dotted')
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauCNNQCalibrator'+sparsityTag+'Pruning_plots/responses_comparison_quantization.pdf')
    plt.close()
    ...
    plt.grid(linestyle='dotted')
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauCNNQCalibrator'+sparsityTag+'Pruning_plots/uncalibrated_DM_responses_comparison.pdf')
    plt.close()

    plt.figure(figsize=(10,10))
    plt.hist(tmp0['Qcalib_pt_pruned']/tmp0['gen_pt'],   bins=np.arange(0,5,0.1), label=DMdict[0]+r' : $\mu$ = %.2f, $\sigma$ =  %.2f' % (np.mean(tmp0['Qcalib_pt_pruned']/tmp0['gen_pt']), np.std(tmp0['Qcalib_pt_pruned']/tmp0['gen_pt'])),      color='lime',  lw=2, density=True, histtype='step', alpha=0.7)
    plt.hist(tmp1['Qcalib_pt_pruned']/tmp1['gen_pt'],   bins=np.arange(0,5,0.1), label=DMdict[1]+r' : $\mu$ = %.2f, $\sigma$ =  %.2f' % (np.mean(tmp1['Qcalib_pt_pruned']/tmp1['gen_pt']), np.std(tmp1['Qcalib_pt_pruned']/tmp1['gen_pt'])),      color='blue', lw=2, density=True, histtype='step', alpha=0.7)
    plt.hist(tmp10['Qcalib_pt_pruned']/tmp10['gen_pt'], bins=np.arange(0,5,0.1), label=DMdict[10]+r' : $\mu$ = %.2f, $\sigma$ =  %.2f' % (np.mean(tmp10['Qcalib_pt_pruned']/tmp10['gen_pt']), np.std(tmp10['Qcalib_pt_pruned']/tmp10['gen_pt'])), color='orange',lw=2, density=True, histtype='step', alpha=0.7)
    plt.hist(tmp11['Qcalib_pt_pruned']/tmp11['gen_pt'], bins=np.arange(0,5,0.1), label=DMdict[11]+r' : $\mu$ = %.2f, $\sigma$ =  %.2f' % (np.mean(tmp11['Qcalib_pt_pruned']/tmp11['gen_pt']), np.std(tmp11['Qcalib_pt_pruned']/tmp11['gen_pt'])), color='fuchsia',lw=2, density=True, histtype='step', alpha=0.7)
    plt.xlabel(r'$p_{T}^{L1 \tau} / p_{T}^{Gen \tau}$')
    plt.ylabel(r'a.u.')
    plt.legend(loc = 'upper right', fontsize=16)
    plt.grid(linestyle='dotted')
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauCNNQCalibrator'+sparsityTag+'Pruning_plots/calibrated_DM_responses_comparison.pdf')
    plt.close()
    ...
    plt.scatter(dfValid['Qcalib_pt_pruned'].head(1000)/dfValid['gen_pt'].head(1000), dfValid['gen_eta'].head(1000), label=r'Calibrated', alpha=0.2, color='green')
    plt.xlabel(r'$p_{T}^{L1 \tau} / p_{T}^{Gen \tau}$')
    ...
    plt.xlim(-0.1,5)
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauCNNQCalibrator'+sparsityTag+'Pruning_plots/response_vs_eta_comparison.pdf')
    plt.close()
    ...
    plt.scatter(dfValid['Qcalib_pt_pruned'].head(1000)/dfValid['gen_pt'].head(1000), dfValid['gen_phi'].head(1000), label=r'Calibrated', alpha=0.2, color='green')
    plt.xlabel(r'$p_{T}^{L1 \tau} / p_{T}^{Gen \tau}$')
    ...
    plt.xlim(-0.1,5)
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauCNNQCalibrator'+sparsityTag+'Pruning_plots/response_vs_phi_comparison.pdf')
    plt.close()
    ...
    plt.scatter(dfValid['Qcalib_pt_pruned'].head(1000)/dfValid['gen_pt'].head(1000), dfValid['gen_pt'].head(1000), label=r'Calibrated', alpha=0.2, color='green')
    plt.xlabel(r'$p_{T}^{L1 \tau} / p_{T}^{Gen \tau}$')
    ...
    plt.xlim(0.0,2.0)
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(outdir+'/TauCNNQCalibrator'+sparsityTag+'Pruning_plots/response_vs_pt_comparison.pdf')
    plt.close()
    ...
    trainL1 = dfTrain.groupby('gen_pt_bin')['Qcalib_pt_pruned'].mean()
    validL1 = dfValid.groupby('gen_pt_bin')['Qcalib_pt_pruned'].mean()
    trainL1std = dfTrain.groupby('gen_pt_bin')['Qcalib_pt_pruned'].std()
    validL1std = dfValid.groupby('gen_pt_bin')['Qcalib_pt_pruned'].std()
    ...
    plt.grid()
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(indir+'/TauCNNQCalibrator'+sparsityTag+'Pruning_plots/GenToCalibL1_pt.pdf')
    plt.close()
    ...
    plt.grid()
    mplhep.cms.label('Phase-2 Simulation', data=True, rlabel='14 TeV, 200 PU')
    plt.savefig(indir+'/TauCNNQCalibrator'+sparsityTag+'Pruning_plots/GenToUncalinL1_pt.pdf')
    plt.close()
...
#    plt.savefig(outdir+'/TauCNNQCalibrator'+sparsityTag+'Pruning_plots/shap0.pdf')
#    plt.close()
#
#    plt.figure(figsize=(10,10))
...
#    plt.savefig(outdir+'/TauCNNQCalibrator'+sparsityTag+'Pruning_plots/shap1.pdf')
#    plt.close()
